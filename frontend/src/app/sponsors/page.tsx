import Navbar from '@/components/layout/Navbar';
import Footer from '@/components/layout/Footer';
import SponsorsSection from '@/components/sections/SponsorsSection';
import { FadeIn } from '@/components/animations/FadeIn';
import type { Metadata } from 'next';

export const metadata: Metadata = {
  title: 'Sponsors',
  description: 'Our partners and sponsors for PRISMTECH 2026. Explore sponsorship opportunities.',
};

export default function SponsorsPage() {
  return (
    <>
      <Navbar />
      <main id="main-content" className="pt-20">
        <section
          className="relative py-20 overflow-hidden"
          style={{ background: 'var(--color-surface-0)' }}
        >
          <div
            className="absolute inset-0 pointer-events-none"
            style={{
              background: 'radial-gradient(ellipse at 50% 0%, rgba(245, 158, 11, 0.12) 0%, transparent 60%)',
            }}
          />
          <div className="container relative z-10 text-center">
            <FadeIn>
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium uppercase tracking-widest text-white/50 glass border border-white/10 mb-6">
                <span className="w-1.5 h-1.5 rounded-full bg-[#f59e0b]" />
                Partnerships
              </div>
              <h1 className="font-display font-800 text-5xl sm:text-6xl text-white mb-5">
                Our <span className="text-gradient-violet-gold">Sponsors</span>
              </h1>
              <p className="text-white/50 text-lg max-w-2xl mx-auto">
                PRISMTECH is made possible by forward-thinking organizations that believe in empowering
                the next generation of engineers and innovators.
              </p>
            </FadeIn>
          </div>
        </section>

        <div className="pb-20" style={{ background: 'var(--color-surface-1)' }}>
          <SponsorsSection />
        </div>
      </main>
      <Footer />
    </>
  );
}
